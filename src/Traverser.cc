...
bool Traverser::traverseExpression(std::shared_ptr<Expression> node)
{
    ...
    switch (node->getType())
    {
        ...
        case Expression::Type::BinaryOperator:
        {
            ...
            if (shouldContinue(result))
            {
                if (!traverseExpression(op->getLeft()) || !traverseExpression(op->getRight()))
                {
                    onNodeExited(node, result);
                    return false;
                }
        ...
        case Expression::Type::BracketOperator:
        {
            ...
            if (shouldContinue(result))
            {
                if (!traverseExpression(op->getContext()) || !traverseExpression(op->getInnerRoot()))
                {
                    onNodeExited(node, result);
                    return false;
                }
    ...
    switch (node->getType())
    {
        case Declaration::Type::Class:
        {
            ...
            if (shouldContinue(result))
            {
                ...
                bool genericTraverseResult =
                    !classType->getGenericRoot() || traverseStatement(classType->getGenericRoot());

                if (!genericTraverseResult || !traverseScope(classType->getBody()))
                {
                    onNodeExited(node, result);
                    return false;
                }
...
Traverser::Result Traverser::onCustomNode(std::shared_ptr<Node>)
{
    return Result::NotHandled;
}

Traverser::Result Traverser::onScope(std::shared_ptr<Scope>)
{
    return Result::NotHandled;
}

Traverser::Result Traverser::onFunction(std::shared_ptr<Function>)
{
    return Result::NotHandled;
}

Traverser::Result Traverser::onClassType(std::shared_ptr<ClassType>)
{
    return Result::NotHandled;
}

Traverser::Result Traverser::onExpressionRoot(std::shared_ptr<Expression::Root>)
{
    return Result::NotHandled;
}

Traverser::Result Traverser::onTypeReference(std::shared_ptr<TypeReference>)
{
    return Result::NotHandled;
}

Traverser::Result Traverser::onBinaryOperator(std::shared_ptr<BinaryOperator>)
{
    return Result::NotHandled;
}

Traverser::Result Traverser::onUnaryOperator(std::shared_ptr<UnaryOperator>)
{
    return Result::NotHandled;
}

Traverser::Result Traverser::onBracketOperator(std::shared_ptr<BracketOperator>)
{
    return Result::NotHandled;
}

Traverser::Result Traverser::onIdentifier(std::shared_ptr<Identifier>)
{
    return Result::NotHandled;
}

Traverser::Result Traverser::onInteger(std::shared_ptr<Integer>)
{
    return Result::NotHandled;
}

Traverser::Result Traverser::onString(std::shared_ptr<String>)
{
    return Result::NotHandled;
}

Traverser::Result Traverser::onVariable(std::shared_ptr<Variable>)
{
    return Result::NotHandled;
}

Traverser::Result Traverser::onReturn(std::shared_ptr<Return>)
{
    return Result::NotHandled;
}
