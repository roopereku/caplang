...
bool Token::isReservedIdentifier(cap::ParserContext& ctx) const
{
    ...
    return (source.match(*this, L"func") || source.match(*this, L"type") || source.match(*this, L"enum"));
}
...
Token Token::parse(ParserContext& ctx, Token token)
{
    ...
    bool shorted =
        (token.setTypeIfMoved(ctx, i, &Token::parseBracket) || token.setTypeIfMoved(ctx, i, &Token::parseNumeric) ||
         token.setTypeIfMoved(ctx, i, &Token::parseString) || token.setTypeIfMoved(ctx, i, &Token::parseIdentifier) ||
         token.setTypeIfMoved(ctx, i, &Token::parseComment) || token.setTypeIfMoved(ctx, i, &Token::parseOperator) ||
