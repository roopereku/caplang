...
void checkLocation(const cap::SourceLocation& loc, Token::Type type, std::wstring_view value, size_t index,
                   size_t length)
{
    ASSERT_EQ(std::string_view(loc.getToken().getTypeString()), std::string_view(Token::getTypeString(type)));
